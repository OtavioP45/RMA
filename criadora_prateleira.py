exec(open('removedora_arquivos.py').read())
#-------------------CRIAÇÃO DO LAUNCH----------------------
launchInicio = "<launch>\n\
\n\
...
def retangulo(vetor, Xinicio, Yinicio):
    for j in range(1):
        for i in range(5):
            # print('x',Xinicio+i)
            # print(Yinicio+j)
            vetor[math.floor(Xinicio+i)][math.floor(Yinicio+j-1)] = 0
    return vetor
#-------------------------------------------------------------Main------------------------------------------------------------------
...
for qb in range(quantidade_blocos):
  ...
  for qf in range(quantidade_fileiras):
      for cf in range(comprimento_fileira):
          arquivoMundo.write(shelf(Xinicial + cf*2.428+qb*(comprimento_fileira*3+tam_corredorVertical),y,0,count,p1))
          Mapa = retangulo(Mapa, (Xinicial-1.6) + cf*2.4+qb*(comprimento_fileira*3+tam_corredorVertical), (y-0.425) )
          if p1:
            if cf == 0:
              checkpoints.append([Xinicial + qb*(comprimento_fileira*3+tam_corredorVertical)-1.2,y+3.5])
            # if cf == comprimento_fileira-1:
            #   checkpoints.append(Xinicial + cf*2.428+qb*(comprimento_fileira*3+tam_corredor)+1.2,y+3.5)

          if andar2:
            arquivoMundo.write(shelf(Xinicial + cf*2.428+qb*(comprimento_fileira*3+tam_corredorVertical),y,3.59,count*-1,p1))
          for qp in range(quantidade_prateleiras):
              for nc in range(caixas_prateleira):
                ...
                if random.random() < ocupacao_caixas:
                    aux_x = random.uniform(-0.25, 0.3)
                    x_pos = Xinicial + (cf*2.428) + aux_x + qb*(comprimento_fileira*3+tam_corredorVertical) + (nc%2)*1.2 - 0.6
...
arquivoProdutos = open("localizacao_produtos.csv", "w")
...
arquivoCheckpoints = open("localizacao_checkpoints.csv", "w")
arquivoCheckpoints.write('X, Y, Z\n')
for i in range(len(checkpoints)):
  arquivoCheckpoints.write(str(checkpoints[i][0]) + ',' + str(checkpoints[i][1]) + ',3\n')
